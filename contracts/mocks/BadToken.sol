...
contract BadToken is LibNote {
	...
	modifier auth {
		require(wards[msg.sender] == 1, "Bdt/not-authorized");
		_;
	}
    ...
    constructor(uint256 chainId_) {
        ...
        dailyBDTLimit = 100000000000000000000000;
    }
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
      ...
      require(balanceOf[src] >= wad, "Bdt/insufficient-balance");
        if (src != msg.sender && _allowance( src, msg.sender ) != type(uint256).min) {
            require(_allowance( src, msg.sender ) >= wad, "Bdt/insufficient-allowance");
    ...
    function adjustBdtlyBDTLimit(uint _limit) external auth {
        dailyBDTLimit = _limit;
    }

    function mint(address usr, uint wad) external {

      if(wards[msg.sender] == 0) {
        require(add(wad, daiMintedToday[msg.sender]) <= dailyBDTLimit || sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= dailyBDTLimit, "Over daily BDT Limit");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "Bdt/insufficient-balance");
        if (usr != msg.sender && _allowance( usr, msg.sender ) != type(uint256).min) {
            require(_allowance( usr, msg.sender ) >= wad, "Bdt/insufficient-allowance");
