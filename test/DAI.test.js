    ...
    describe('for a non zero account', function () {
      ...
      it('emits Transfer event', async function () {
        const event = expectEvent(
          this.receipt,
          'Transfer',
          { src: ZERO_ADDRESS, dst: recipient, wad: amount },
        );
    ...
    describe('for a non zero account', function () {
      it('rejects burning more than balance', async function () {
        await expectRevert(this.token.burn(
          initialHolder, initialSupply.addn(1), { from: initialHolder }),
      ...
      const describeBurn = function (description, amount) {
        describe(description, function () {
          beforeEach('burning', async function () {
            this.receipt = await this.token.burn(initialHolder, amount, { from: initialHolder });
          });
          ...
          it('emits Transfer event', async function () {
            const event = expectEvent(
              this.receipt,
              'Transfer',
              { src: initialHolder, dst: ZERO_ADDRESS, wad: amount },
            );
