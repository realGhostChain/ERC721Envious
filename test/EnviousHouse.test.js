...
contract('EnviousHouse', function (accounts) {
	...
	it('comissions are correct', async function () {
		await this.house.registerCollection(this.basicCollection.address, this.baseToken.address, new BN('1'), new BN('1'), { value: this.amount });
		await this.house.registerCollection(this.enviousCollection.address, ZERO_ADDRESS, new BN('0'), new BN('0'), { value: this.amount });
	...
	it('number of total collections is correct', async function () {
		...
		await this.house.registerCollection(this.basicCollection.address, ZERO_ADDRESS, new BN('0'), new BN('0'), { value: this.amount });
	...
	it('could not set if disperse amount is low', async function () {
		await expectRevert(
			this.house.registerCollection(this.basicCollection.address, ZERO_ADDRESS, new BN('0'), new BN('0')),
			'low amount'
		);
	});
	...
	it('could not set if commission exists but no community token provided', async function () {
		await expectRevert(
			this.house.registerCollection(this.basicCollection.address, ZERO_ADDRESS, new BN('1'), new BN('0'), { value: this.amount }),
			'no community token provided'
		);
		await expectRevert(
			this.house.registerCollection(this.basicCollection.address, ZERO_ADDRESS, new BN('0'), new BN('1'), { value: this.amount }),
			'no community token provided'
		);
	});
	...
	it('revert if collection already registered', async function () {
		...
		await expectRevert(
			this.house.registerCollection(this.enviousCollection.address, ZERO_ADDRESS, new BN('0'), new BN('1'), { value: this.amount }),
			'collection exists'
		);
	});

	it('revert if address is not ERC721', async function () {
		await expectRevert.unspecified(this.house.registerCollection(this.baseToken.address, ZERO_ADDRESS, new BN('0'), new BN('0'), { value: this.amount }));
	});

	it('could not set ghost addresses from arbitrary address', async function () {
		await expectRevert.unspecified(this.house.setGhostAddresses(this.baseToken.address, this.baseToken.address, { from: other }))
	});
	...
	describe('collateralize', function () {
		beforeEach(async function () {
			...
			await this.house.registerCollection(this.basicCollection.address, this.communityToken.address, new BN('1000'), new BN('1000'), { value: this.amount });
			await this.house.registerCollection(this.enviousCollection.address, ZERO_ADDRESS, new BN('0'), new BN('0'), { value: this.amount });
		});

		it('revert for empty collection', async function () {
			await expectRevert(
				this.house.collateralize(this.badCollection.address, this.tokenId, [this.halfAmount], [this.baseToken.address]),
				'no tokens minted'
			);
		});
		...
		it('with ether', async function () {
			...
			await this.house.collateralize(this.basicCollection.address, this.tokenId, [this.halfAmount], [ZERO_ADDRESS], { value: this.halfAmount });
			const taken = this.halfAmount.mul(new BN('1000')).div(new BN('100000'));
			const newBalance = this.halfAmount.sub(taken);
		...
		it('emits event Collateralized', async function () {
			expectEvent(
				await this.house.collateralize(this.basicCollection.address, this.tokenId, [this.halfAmount], [this.baseToken.address]),
				'Collateralized',
				{ collection: this.basicCollection.address, tokenId: this.tokenId, amount: this.halfAmount, tokenAddress: this.baseToken.address }
			);
		});

		it('revert if lengths not match', async function () {
			await expectRevert(
				this.house.collateralize(this.basicCollection.address, this.tokenId, [this.halfAmount, this.halfAmount], [this.baseToken.address]),
				'lengths not match'
			);
		});
		...
		it('with multiple assets', async function () {
			...
			await this.house.collateralize(
				this.basicCollection.address, 
				this.tokenId, 
				[this.halfAmount, this.halfAmount], 
				[ZERO_ADDRESS, this.baseToken.address], 
				{ gasPrice: 63632263, value: this.halfAmount }
			);
		...
		it('if collateral exists, could not register', async function () {
			await this.house.collateralize(this.basicCollection.address, this.tokenId, [this.halfAmount], [this.baseToken.address]);
			await expectRevert(
				this.house.registerCollection(this.basicCollection.address, ZERO_ADDRESS, new BN('0'), new BN('0'), { value: this.amount }),
				'collection exists'
			);
		});
		...
		describe('for envious', function () {
			it('revert from house', async function () {
				await expectRevert(
					this.house.collateralize(this.enviousCollection.address, this.tokenId, [this.halfAmount], [this.baseToken.address]),
					'already envious'
				);
			});
	...
	describe('disperse', function () {
		...
		it('revert for empty collection', async function () {
			await expectRevert(
				this.house.disperse(this.badCollection.address, [this.halfAmount], [this.baseToken.address]),
				'no tokens minted'
			);
		});
		...
		it('with ether', async function () {
			...
			await this.house.disperse(this.basicCollection.address, [this.halfAmount], [ZERO_ADDRESS], { gasPrice: 63632263, value: this.halfAmount });
		...
		it('emits event Dispersed', async function () {
			expectEvent(
				await this.house.disperse(this.basicCollection.address, [this.halfAmount], [this.baseToken.address]),
				'Dispersed',
				{ collection: this.basicCollection.address, amount: this.halfAmount, tokenAddress: this.baseToken.address }
			);
		});

		it('revert if lengths not match', async function () {
			await expectRevert(
				this.house.disperse(this.basicCollection.address, [this.halfAmount, this.halfAmount], [this.baseToken.address]),
				'lengths not match'
			);
		});
		...
		describe('for envious', function () {
			it('revert from house', async function () {
				await expectRevert(
					this.house.disperse(this.enviousCollection.address, [this.halfAmount], [this.baseToken.address]),
					'already envious'
				);
			});
	...
	describe('uncollateralize', function () {
		beforeEach(async function () {
			...
			await this.house.registerCollection(this.basicCollection.address, this.communityToken.address, new BN('0'), new BN('1000'), { value: this.amount });
			await this.house.registerCollection(this.enviousCollection.address, ZERO_ADDRESS, new BN('0'), new BN('0'), { value: this.amount });
		...
		it('with ether', async function () {
			...
			const tx = await this.house.uncollateralize(this.basicCollection.address, this.tokenId, [this.halfAmount], [ZERO_ADDRESS], { gasPrice: 63632263 });
		...
		it('emits event Uncollateralized', async function () {
			...
			expectEvent(
				await this.house.uncollateralize(this.basicCollection.address, this.tokenId, [this.halfAmount], [this.baseToken.address]),
				'Uncollateralized',
				{ collection: this.basicCollection.address, tokenId: this.tokenId, amount: result, tokenAddress: this.baseToken.address }
			);
		});

		it('revert if lengths not match', async function () {
			await expectRevert(
				this.house.uncollateralize(this.basicCollection.address, this.tokenId, [this.halfAmount, this.halfAmount], [this.baseToken.address]),
				'lengths not match'
			);
		});
		...
		it('only for owner', async function () {
			await expectRevert(
				this.house.uncollateralize(this.basicCollection.address, this.tokenId, [this.halfAmount], [this.baseToken.address], { from: other }),
				'not token owner'
			);
		});
		...
		it('with multiple assets', async function () {
			...
			const tx = await this.house.uncollateralize(
				this.basicCollection.address, 
				this.tokenId, 
				[this.halfAmount, this.halfAmount], 
				[ZERO_ADDRESS, this.baseToken.address], 
		...
		it('revert if not registered', async function () {
			await expectRevert(
				this.house.uncollateralize(this.badCollection.address, this.tokenId, [this.halfAmount], [this.baseToken.address]),
		...
		describe('for envious', function () {
			it('revert from house', async function () {
				await expectRevert(
					this.house.uncollateralize(this.enviousCollection.address, this.tokenId, [this.halfAmount], [this.baseToken.address]),
					'already envious'
				);
			});
	...
	describe('harvest', function () {
		beforeEach(async function () {
			...
			await this.house.registerCollection(this.basicCollection.address, this.communityToken.address, new BN('1000'), new BN('0'), { value: this.amount });
			await this.house.registerCollection(this.enviousCollection.address, ZERO_ADDRESS, new BN('0'), new BN('0'), { value: this.amount });
		...
		it('with ether', async function () {
			...
			const tx = await this.house.harvest(this.basicCollection.address, [this.halfAmount], [ZERO_ADDRESS], { gasPrice: 63632263 });
			let gasUsed = new BN(tx.receipt.gasUsed);
			gasUsed = gasUsed.mul(new BN('63632263'));
		...
		it('emits event Harvested', async function () {
			...
			expectEvent(
				await this.house.harvest(this.basicCollection.address, [this.halfAmount], [this.baseToken.address]),
				'Harvested',
				{ collection: this.basicCollection.address, scaledAmount: scaled, amount: this.halfAmount, tokenAddress: this.baseToken.address }
			);
		});

		it('revert if lengths not match', async function () {
			await expectRevert(
				this.house.harvest(this.basicCollection.address, [this.halfAmount, this.halfAmount], [this.baseToken.address]),
				'lengths not match'
			);
		});
		...
		it('with multiple assets', async function () {
			...
			const tx = await this.house.harvest(
				this.basicCollection.address,
				[this.halfAmount, this.halfAmount], 
				[ZERO_ADDRESS, this.baseToken.address], 
				{ gasPrice: 63632263 }
		...
		it('revert if not registered', async function () {
			await expectRevert(
				this.house.harvest(this.badCollection.address, [this.halfAmount], [this.baseToken.address]),
				'collection not exists'
			);
		});
		...
		describe('for envious', function () {
			it('revert from house', async function () {
				await expectRevert(
					this.house.harvest(this.enviousCollection.address, [this.halfAmount], [this.baseToken.address]),
					'already envious'
				);
			});
		});
	...
	describe('getDiscountedCollateral', function () {
		...
		it('with token', async function () {
			...
			await this.house.getDiscountedCollateral(
				this.basicCollection.address, 
				new BN('1'),
		...
		it('collection should be registered', async function () {
			await expectRevert(
				this.house.getDiscountedCollateral(
					this.badCollection.address, 
					new BN('1'),
					this.baseToken.address,
					this.tokenId,
					this.halfAmount, 
		...
		describe('for envious', function () {
			it('revert from house', async function () {
				await expectRevert(
					this.house.getDiscountedCollateral(
						this.enviousCollection.address, 
						new BN('1'),
						this.baseToken.address,
						this.tokenId,
						this.halfAmount, 
			...
			it('envious collection info is visible through house', async function () {
				...
				await this.enviousCollection.getDiscountedCollateral(
					new BN('1'),
					this.baseToken.address,
					this.tokenId,
					this.halfAmount, 
	...
	describe('claimDiscountedCollateral', function () {
		...
		it('collection should be registered', async function () {
			await expectRevert(
				this.house.claimDiscountedCollateral(this.badCollection.address, this.tokenId, [new BN('0')]),
				'collection not exists'
			);
		});
		...
		describe('for envious', function () {
			it('revert from house', async function () {
				await expectRevert(
					this.house.claimDiscountedCollateral(this.enviousCollection.address, this.tokenId, [new BN('0')]),
					'already envious'
				);
			});
