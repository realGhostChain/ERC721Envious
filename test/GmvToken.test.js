...
} = require("./ERC20/ERC20Burnable.behavior");

const ERC20Mock = artifacts.require("GhostMcAfeeVision");

contract("GhostMcAfeeVision Token (GMV)", function(accounts) {
    const [ initialHolder, recipient, anotherAccount ] = accounts;
    const name = "Ghost McAfee Vision";
    const symbol = "GMV";
    ...
    it("can renew super minter", async function() {
        await this.token.renewSuperMinter(anotherAccount);
        await expectRevert(this.token.mint(initialHolder, initialSupply), "not a super minter");
    });
    it("only super user can renew super minter", async function() {
        await expectRevert(this.token.renewSuperMinter(anotherAccount, {
            from: anotherAccount
        }), "not a super user");
    });
            ...
            function shouldDecreaseApproval(amount) {
                describe("when there was no approved amount before", function() {
                    it("reverts", async function() {
                        await expectRevert(this.token.decreaseAllowance(spender, amount, {
                            from: initialHolder
                ...
                describe("when the spender had an approved amount", function() {
                    ...
                    it("emits an approval event", async function() {
                        expectEvent(await this.token.decreaseAllowance(spender, approvedAmount, {
                            from: initialHolder
                        }), "Approval", {
                            owner: initialHolder,
                            spender: spender,
                    ...
                    it("reverts when more than the full allowance is removed", async function() {
                        await expectRevert(this.token.decreaseAllowance(spender, approvedAmount.addn(1), {
                            from: initialHolder
                        }), "ERC20: decreased allowance below zero");
                    });
                });
    ...
    describe("increase allowance", function() {
        ...
        describe("when the spender is not the zero address", function() {
            ...
            describe("when the sender has enough balance", function() {
                it("emits an approval event", async function() {
                    expectEvent(await this.token.increaseAllowance(spender, amount, {
                        from: initialHolder
                    }), "Approval", {
                        owner: initialHolder,
                        spender: spender,
        ...
        describe("when the spender is the zero address", function() {
            const spender = ZERO_ADDRESS;
            it("reverts", async function() {
                await expectRevert(this.token.increaseAllowance(spender, amount, {
                    from: initialHolder
