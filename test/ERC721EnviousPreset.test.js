...
const { shouldBehaveLikeERC721Pausable } = require('./ERC721/ERC721Pausable.behavior');
const { shouldBehaveLikeERC721Burnable } = require('./ERC721/ERC721Burnable.behavior');
...
contract('ERC721EnviousPreset', function (accounts) {
    ...
    const incomingFee = new BN('10000');   // 10%
    const outcomingFee = new BN('5000');   // 5%
	...
	describe('community addresses', function () {
		it('harvest token could not be zero address', async function () {
			await expectRevert(
				this.token.changeCommunityAddresses(ZERO_ADDRESS, this.blackHole.address),
				'zero address'
			);
		});
		...
		it('revert if called from arbitrary address', async function () {
			await expectRevert(
				this.token.changeCommunityAddresses(this.harvestToken.address, this.blackHole.address, { from: accounts[1] }),
				'incorrect role'
			);
		});
	});
