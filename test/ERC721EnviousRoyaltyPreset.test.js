...
contract("ERC721EnviousRoyaltyPreset", function(accounts) {
    ...
    const feeForAddress = new BN("1000");
    const feeForSmart = new BN("2000");
    const incomingFee = new BN("1000");
    const outcomingFee = new BN("2000");
    ...
    beforeEach(async function() {
        ...
        this.token = await ERC721EnviousMock.new(name, symbol, uri, feeForAddress, feeForSmart);
    ...
    describe("reseting availability", function() {
        it("arbitrary address could not change ghost related addresses", async function() {
            await expectRevert(this.token.setGhostAddresses(ghst, bonding, {
                from: other
            }), "Ownable: caller is not the owner");
        });
        it("arbitrary address could not change commissions", async function() {
            await expectRevert(this.token.changeCommissions(new BN("0"), new BN("0"), {
                from: other
            }), "Ownable: caller is not the owner");
        });
        it("arbitrary address could not change royalties", async function() {
            await expectRevert(this.token.changeRoyalties(feeForAddress, feeForSmart, {
                from: other
            }), "Ownable: caller is not the owner");
        });
        it("ghost token address could not be zero address", async function() {
            await expectRevert(this.token.setGhostAddresses(ZERO_ADDRESS, bonding), "zero address found");
        });
        it("ghost bonding address could not be zero address", async function() {
            await expectRevert(this.token.setGhostAddresses(ghst, ZERO_ADDRESS), "zero address found");
        });
    ...
    describe("minting", function() {
        it("arbitrary address could not mint tokens", async function() {
            await expectRevert(this.token.mint(holder, {
                from: other
            }), "error");
        });
    ...
    shouldBehaveLikeERC721Discounted(...accounts);
    shouldSupportInterfaces([ "ERC721Envious", "ERC721EnviousRoyalty" ]);
});