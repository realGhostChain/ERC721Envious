    ...
    context("like a ERC721Envious", function() {
        ...
        describe("collateralization", function() {
            ...
            it("with ether", async function() {
                ...
                await expectRevert.unspecified(this.token.collateralTokens(this.tokenId, new BN("0")));
                await this.token.collateralize(this.tokenId, [ this.amount ], [ ZERO_ADDRESS ], {
                    value: this.amount
                });
            ...
            it("emits an event Collateralize", async function() {
                expectEvent(await this.token.collateralize(this.tokenId, [ this.amount ], [ this.baseToken.address ]), "Collateralized", {
            ...
            it("with multiple tokens", async function() {
                ...
                await this.token.collateralize(this.tokenId, amounts, tokens, {
                    value: this.amount
                });
            ...
            it("with multiple tokens while array lengths should match", async function() {
                ...
                await expectRevert(this.token.collateralize(this.tokenId, wrongAmounts, tokens), "ERC721Envious: lengths not match");
                await expectRevert(this.token.collateralize(this.tokenId, amounts, wrongTokens), "ERC721Envious: lengths not match");
            });
            ...
            it("available for any arbitrary address", async function() {
                await this.baseToken.mint(stranger, this.amount, {
                    from: stranger
                });
                await this.baseToken.approve(this.token.address, this.amount, {
                    from: stranger
                });
                expectEvent(await this.token.collateralize(this.tokenId, [ this.amount ], [ this.baseToken.address ], {
                    from: stranger
                }), "Collateralized", {
                    tokenId: this.tokenId,
                    amount: this.amount,
            ...
            it("revert if amount not match to ether sent value", async function() {
                await expectRevert.unspecified(this.token.collateralize(this.tokenId, [ this.amount ], [ ZERO_ADDRESS ], {
                    value: new BN("1")
                }));
            });
            it("extra ether will be send back", async function() {
                const eth = await balance.current(holder);
                const tx = await this.token.collateralize(this.tokenId, [ this.amount ], [ ZERO_ADDRESS ], {
                    value: this.amount.mul(new BN("2")),
                    gasPrice: 63632263
                });
        ...
        describe("uncollateralization", function() {
            ...
            it("of ether", async function() {
                ...
                const eth = await balance.current(holder);
                let tx = await this.token.uncollateralize(this.tokenId, [ this.backAmount ], [ ZERO_ADDRESS ], {
                    gasPrice: 63632263
                });
                let gasUsed = new BN(tx.receipt.gasUsed);
                gasUsed = gasUsed.mul(new BN("63632263"));
            ...
            it("emits an event Uncollateralize", async function() {
                expectEvent(await this.token.uncollateralize(this.tokenId, [ this.backAmount ], [ this.baseToken.address ]), "Uncollateralized", {
                    tokenId: this.tokenId,
                    amount: this.backAmount.sub(this.backAmount.mul(this.uncollateralFee).div(new BN("100000"))),
                    tokenAddress: this.baseToken.address
                });
            });
            ...
            it("forbidden for arbitrary address", async function() {
                await expectRevert(this.token.uncollateralize(this.tokenId, [ this.backAmount ], [ this.baseToken.address ], {
                    from: stranger
            ...
            it("revert if amounts not match with addresses", async function() {
                ...
                const tokens = [ this.baseToken.address, this.daiToken.address, this.usdtToken.address, this.rebaseToken.address, ZERO_ADDRESS ];
                await expectRevert(this.token.uncollateralize(this.tokenId, wrongAmounts, tokens), "ERC721Envious: lengths not match");
                await expectRevert(this.token.uncollateralize(this.tokenId, amounts, wrongTokens), "ERC721Envious: lengths not match");
            });
        ...
        describe("disperse", function() {
            ...
            it("of ether", async function() {
                ...
                await expectRevert.unspecified(this.token.disperseTokens(new BN("0")));
                let tx = await this.token.disperse([ this.amount ], [ ZERO_ADDRESS ], {
                    value: this.amount,
                    gasPrice: 63632263
                });
                let gasUsed = new BN(tx.receipt.gasUsed);
            ...
            it("of ether, rest is returned back", async function() {
                ...
                await this.token.disperse([ this.amount ], [ ZERO_ADDRESS ], {
            ...
            it("emits event Dispersed", async function() {
                expectEvent(await this.token.disperse([ this.amount ], [ this.baseToken.address ]), "Dispersed", {
                    tokenAddress: this.baseToken.address,
                    amount: this.amount
                });
            });
            it("with direct sending of ether to smart contract", async function() {
                expect(await this.token.disperseBalance(ZERO_ADDRESS)).to.be.bignumber.eq(new BN("0"));
                expectEvent(await this.token.sendTransaction({
                    from: holder,
            ...
            it("disperse of multiple tokens", async function() {
                ...
                const amounts = [ this.amount, this.amount, this.amount, this.amount, this.amount ];
                const tokens = [ this.baseToken.address, this.daiToken.address, this.usdtToken.address, this.rebaseToken.address, ZERO_ADDRESS ];
                await this.token.disperse(amounts, tokens, {
                    value: this.amount
                });
                expect(await this.token.disperseTokens(new BN("0"))).to.be.eq(this.baseToken.address);
            ...
            it("any arbitrary address can disperse", async function() {
                ...
                expectEvent(await this.token.disperse([ this.amount ], [ this.baseToken.address ], {
                    from: stranger
                }), "Dispersed", {
                    tokenAddress: this.baseToken.address,
                    amount: this.amount
        ...
        describe("harvest", function() {
            beforeEach(async function() {
                if (this.collateralFee.toString() === "0" && this.uncollateralFee.toString() === "0") {
                    ...
                    await this.harvestToken.approve(this.token.address, this.amount, {
                        from: stranger
                    });
                }
            });
            it("of basic token", async function() {
                ...
                await this.token.harvest([ this.amount ], [ this.baseToken.address ], {
                    from: stranger
                });
            ...
            it("of DAI mock", async function() {
                ...
                await this.token.harvest([ this.amount ], [ this.daiToken.address ], {
                    from: stranger
                });
            ...
            it("of USDT mock", async function() {
                ...
                await this.token.harvest([ this.amount ], [ this.usdtToken.address ], {
                    from: stranger
                });
            ...
            it("of rebasing token", async function() {
                ...
                await this.token.harvest([ this.amount ], [ this.rebaseToken.address ], {
                    from: stranger
                });
            ...
            it("of ether", async function() {
                ...
                expect(await this.harvestToken.balanceOf(this.blackHole.address)).to.be.bignumber.eq(new BN("0"));
                if (this.uncollateralFee.toString() !== "0") {
                    await this.token.uncollateralize(this.tokenId, [ this.amount.sub(collected) ], [ ZERO_ADDRESS ], {
                        gasPrice: 63632263
                    });
                    collected = collected.add(this.amount.sub(collected).mul(this.uncollateralFee).div(new BN("100000")));
                    tokenBalance = new BN("0");
                ...
                const tx = await this.token.harvest([ this.amount ], [ ZERO_ADDRESS ], {
                    from: stranger,
                    gasPrice: 63632263
                });
                let gasUsed = new BN(tx.receipt.gasUsed);
            ...
            it("of multiple tokens", async function() {
                ...
                const amounts = [ new BN("100000000000000000"), new BN("100000000000000000"), new BN("100000"), new BN("100000000"), new BN("100000000000000000") ];
                const tokens = [ this.baseToken, this.daiToken, this.usdtToken, this.rebaseToken, ZERO_ADDRESS ];
                ...
                let etherBalanceStranger = await balance.current(stranger);
                let tx = await this.token.collateralize(this.tokenId, amounts, tokenAddresses, {
                ...
                tx = await this.token.harvest(Array(5).fill(partHarvest), tokenAddresses, {
                    from: stranger,
                    gasPrice: 63632263
                });
            ...
            it("emits Harvested event", async function() {
                ...
                const scaled = await this.token.getAmount(this.amount, this.baseToken.address, {
                    from: stranger
                });
                expectEvent(await this.token.harvest([ this.amount ], [ this.baseToken.address ], {
                    from: stranger
                }), "Harvested", {
                    tokenAddress: this.baseToken.address,
                    amount: this.amount,
            ...
            it("revert if amount exceeds actual balance", async function() {
                await this.token.collateralize(this.tokenId, [ this.amount ], [ this.baseToken.address ]);
                await this.token.collateralize(this.tokenId, [ this.amount ], [ this.daiToken.address ]);
                await expectRevert.unspecified(this.token.harvest([ this.amount ], [ this.baseToken.address ], {
                    from: stranger
                }));
            });
        });
        describe("_disperse", function() {
            beforeEach(async function() {
                ...
                await this.token.disperse([ this.actualAmount, this.actualAmount ], [ this.baseToken.address, ZERO_ADDRESS ], {
            ...
            it("amount from dispersed goes to actual collateral", async function() {
                await this.token.collateralize(this.tokenId, [ this.actualAmount, this.actualAmount ], [ this.baseToken.address, ZERO_ADDRESS ], {
                    value: this.actualAmount
                });
                expect(await this.token.disperseBalance(this.baseToken.address)).to.be.bignumber.eq(this.actualAmount);
                expect(await this.token.disperseBalance(ZERO_ADDRESS)).to.be.bignumber.eq(this.actualAmount);
            ...
            it("amount in ether from dispersed could be uncollateralized", async function() {
                ...
                const part = this.actualAmount.div(new BN("2"));
                const result = part.sub(part.mul(this.uncollateralFee).div(new BN("100000")));
                let tx = await this.token.uncollateralize(this.tokenId, [ part ], [ ZERO_ADDRESS ], {
                    gasPrice: 63632263
                });
                let gasUsed = new BN(tx.receipt.gasUsed);
                gasUsed = gasUsed.mul(new BN("63632263"));
                ...
                tx = await this.token.uncollateralize(this.otherTokenId, [ part ], [ ZERO_ADDRESS ], {
                    gasPrice: 63632263
                });
                gasUsed = new BN(tx.receipt.gasUsed);
                gasUsed = gasUsed.mul(new BN("63632263"));
        ...
        describe("bad collateral token", function() {
            ...
            it("revert multi collateral if token do not contain decimals", async function() {
                await expectRevert.unspecified(this.token.collateralize(this.tokenId, [ this.amount, this.amount, this.amount ], [ this.baseToken.address, this.daiToken.address, this.badToken.address ]));
            });
            it("revert multi collateral with ether if token do not contain decimals", async function() {
                await expectRevert.unspecified(this.token.collateralize(this.tokenId, [ this.amount, this.amount, this.amount ], [ this.baseToken.address, ZERO_ADDRESS, this.badToken.address ], {
                    value: this.amount
                }));
            });
            ...
            it("revert multi disperse if token do not contain decimals", async function() {
                await expectRevert.unspecified(this.token.disperse([ this.amount, this.amount, this.amount ], [ this.baseToken.address, this.daiToken.address, this.badToken.address ]));
            });
            it("revert multi disperse with ether if token do not contain decimals", async function() {
                await expectRevert.unspecified(this.token.disperse([ this.amount, this.amount, this.amount ], [ this.baseToken.address, ZERO_ADDRESS, this.badToken.address ], {
                    value: this.amount
                }));
            });
        });
    ...
    context("like a ERC721Envious with discount", function() {
        ...
        describe("buy DeFi 2.0 bond", function() {
            ...
            describe("if bonding address is correct", function() {
                ...
                it("discounted collateral emits event Bond", async function() {
                    ...
                    expectEvent.inTransaction(receipt.tx, this.bondingMock, "Bond", {
                        id: this.bondId,
                        amount: this.amount,
                        price: price
                    });
                ...
                it("revert if tokenId does not exist", async function() {
                    await expectRevert(this.token.getDiscountedCollateral(this.bondId, this.quoteToken.address, new BN("0"), this.amount, this.maxPrice), "ERC721: invalid token ID");
                });
                it("revert if bond does not exist", async function() {
                    await expectRevert(this.token.getDiscountedCollateral(new BN("0"), this.quoteToken.address, this.tokenId, this.amount, this.maxPrice), "Depository: market concluded");
                });
                it("revert if bond not available for current maxPrice", async function() {
                    await expectRevert(this.token.getDiscountedCollateral(this.bondId, this.quoteToken.address, this.tokenId, this.amount, new BN("0")), "Depository: market concluded");
                });
                it("revert if bond is over", async function() {
                    await expectRevert(this.token.getDiscountedCollateral(this.bondId, this.quoteToken.address, this.tokenId, new BN("0"), this.maxPrice), "Depository: max size exceeded");
                });
        ...
        describe("claim DeFi 2.0 bond", function() {
            ...
            it("emits Collateralized event", async function() {
                await time.increase(this.bondingTimeOffset + 1);
                expectEvent(await this.token.claimDiscountedCollateral(this.tokenId, [ this.claimIndex ]), "Collateralized", {
                    tokenId: this.tokenId,
                    amount: this.amount,
                    tokenAddress: this.ghostToken.address
                });
            });
    ...
    context("like a ERC721Dynamic", function() {
        ...
        describe("setting ghost related addresses", function() {
            ...
            it("token address must be non-zero", async function() {
                await expectRevert(this.token.setGhostAddresses(ZERO_ADDRESS, this.staking.address), "zero address found");
            });
            it("staking address must be non-zero", async function() {
                await expectRevert(this.token.setGhostAddresses(this.rebaseToken.address, ZERO_ADDRESS), "zero address found");
            });
        ...
        describe("dynamic uri", function() {
            ...
            describe("different range for tokenURI after crossing the edge", function() {
                it("before first edge", async function() {
                    ...
                    expect(parseInt(myUri)).to.be.lte(1e3);
                });
...
function shouldBehaveLikeERC721Vrf(holder, stranger) {
    context("like a VRF v2", function() {
        ...
        it("initializeVRF emits event", async function() {
            const one = new BN("1");
            expectEvent(await this.token.initializeVRF(one, ZERO_BYTES32, one, one, one), "VrfChanged", {
                newSSubscriptionId: one,
                newSKeyHash: ZERO_BYTES32,
                newNumWords: one,
                newCallbackGasLimit: one,
                newRequestConfirmations: one
            });
        });
        describe("collateral random tokens", function() {
            ...
            it("revert if length of arrays not match", async function() {
                await expectRevert(this.token.collateralRandomTokens([ this.amount, this.amount ], [ this.baseToken.address ]), "lengths not match");
            });
            it("revert if denominator is zero", async function() {
                ...
                await expectRevert(this.token.collateralRandomTokens([ this.amount ], [ this.baseToken.address ]), "vrf not prepared");
            });
            ...
            it("could collateral with ether random tokens and return ether surplus", async function() {
                const prevBalance = await balance.current(holder);
                const tx = await this.token.collateralRandomTokens([ this.amount ], [ ZERO_ADDRESS ], {
                    value: this.amount,
                    gasPrice: 63632263
                });
                let gasUsed = new BN(tx.receipt.gasUsed);
        ...
        describe("collateral random amounts", function() {
            ...
            it("revert if vrf not prepared", async function() {
                await this.token.collateralRandomTokens([ this.amount ], [ this.baseToken.address ]);
                await expectRevert(this.token.collateralRandomAmounts([ new BN("1") ], this.amount, this.baseToken.address), "vrf not prepared");
            });
...
module.exports = {
    shouldBehaveLikeERC721Envious: shouldBehaveLikeERC721Envious,
    shouldBehaveLikeERC721Discounted: shouldBehaveLikeERC721Discounted,
    shouldBehaveLikeERC721Dynamic: shouldBehaveLikeERC721Dynamic,
    shouldBehaveLikeERC721Vrf: shouldBehaveLikeERC721Vrf
};