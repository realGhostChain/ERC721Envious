...
function shouldBehaveLikeERC721Pausable(errorPrefix, owner, receiver, operator) {
    context("when token is paused", function() {
        ...
        it("reverts when trying to transferFrom", async function() {
            await expectRevert(this.token.transferFrom(owner, receiver, firstTokenId, {
                from: owner
            }), `${errorPrefix}: token transfer while paused`);
        });
        it("reverts when trying to safeTransferFrom", async function() {
            await expectRevert(this.token.safeTransferFrom(owner, receiver, firstTokenId, {
                from: owner
            }), `${errorPrefix}: token transfer while paused`);
        });
        it("reverts when trying to safeTransferFrom with data", async function() {
            await expectRevert(this.token.methods["safeTransferFrom(address,address,uint256,bytes)"](owner, receiver, firstTokenId, mockData, {
                from: owner
            }), `${errorPrefix}: token transfer while paused`);
        });
        it("reverts when trying to mint", async function() {
            await expectRevert(this.token.mint(receiver), `${errorPrefix}: token transfer while paused`);
        });
        it("reverts when trying to burn", async function() {
            await expectRevert(this.token.burn(firstTokenId), `${errorPrefix}: token transfer while paused`);
        });
...
module.exports = {
    shouldBehaveLikeERC721Pausable: shouldBehaveLikeERC721Pausable
};