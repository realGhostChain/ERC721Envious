...
contract('EnviousHouse', function (accounts) {
	...
	beforeEach(async function () {
		...
		this.collection = await GhostCollection.new(
			enviousName, 
			enviousSymbol, 
			uri,
			edgeValues,
		...
		this.faucet = await GhostFaucet.new(
	...
	it('event on success airdrop is emitted', async function () {
		expectEvent(
			...
			'AssetAirdropped',
			{
				sender: user, friend: defaultAddress, amount: rewardUnit
			}
		);
	});
	...
	it('URI could not be changed by arbitrary address', async function () {
		await expectRevert(this.collection.changeBaseUri('failed', { from: stranger }), 'only for super user');
	});
	...
	it('GHOST addresses could not be changed by arbitrary address', async function () {
		await expectRevert(
			this.collection.setGhostAddresses(this.token.address, stranger, { from: stranger }), 
			'only for super user'
		);
	});
	...
	describe('affiliate usage', function () {
		...
		it('increases reward based on sigmoid', async function () {
			...
			for (let i = 1; i < 5; i++) {
				...
				await this.faucet.sendMeGhostNft(defaultAddress, { value: new BN('1000') })
	...
	describe('multiple self minting', function () {
		...
		it('leads to increasing of collection disperse in ETH', async function () {
			...
			await this.faucet.sendMeGhostNft(defaultAddress, { value: result });
